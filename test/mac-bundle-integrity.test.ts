...
function makeFixture(root: string, appName = "Spiral Buddy.app") {
  ...
  fs.writeFileSync(path.join(macOS, "Spiral Buddy"), "fixture", { mode: 0o755 });
...
describe("macOS update bundle integrity", () => {
  ...
  test("accepts a complete app whose framework links are relative and contained", () => {
    ...
    assert.equal(isValidMacAppBundle(appPath, "Spiral Buddy"), true);
  });

  test("rejects absolute, broken and bundle-escaping framework links", () => {
    for (const target of [
      "/Volumes/Detached DMG/Spiral Buddy.app/Contents/Frameworks/Electron Framework",
      ...
      assert.equal(isValidMacAppBundle(appPath, "Spiral Buddy"), false, target);
    }
  });
  ...
  test("verbatim staging remains launchable after the source bundle disappears", async () => {
    ...
    assert.equal(isValidMacAppBundle(stagedApp, "Spiral Buddy"), true);
    assert.equal(
      fs.readlinkSync(
        path.join(
          stagedApp,
