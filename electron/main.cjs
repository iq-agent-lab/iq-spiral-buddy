...
function expectedUpdateAssetName(version) {
  ...
  if (process.platform === "win32") {
    return `Spiral.Buddy.Setup.${version}.exe`;
  }
  return null;
}
...
function macUpdatePaths(version) {
  return {
    targetApp: "/Applications/Spiral Buddy.app",
    stagedApp: `/Applications/.Spiral Buddy.update-${version}.app`,
    backupApp: `/Applications/.Spiral Buddy.backup-${version}.app`,
    executableName: "Spiral Buddy",
    volumeAppName: "Spiral Buddy.app",
  };
}
...
function cleanupSuccessfulUpdateBackup(version) {
  ...
  const backupPath = path.join(
    "/Applications",
    `.Spiral Buddy.backup-${version}.app`,
  );
  try {
...
async function installUpdateMacOS(version, logPath, asset) {
  ...
  const dmgPath = path.join(
    os.tmpdir(),
    `spiral-buddy-${version}-${process.arch}.dmg`,
  );
  const download = await downloadUpdateAsset({
    asset,
    dest: dmgPath,
